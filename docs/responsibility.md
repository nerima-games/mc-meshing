...
| 流体の高さ / 流れ方向 | **保留（理由を更新。§3.6）** | 参照実装の `greedy-meshing-fluids.ts` + `-fluid-state.ts`（385 LOC）。流体伝播ルール自体は mx-gameplay。**メッシュ側はここのものだが、入力が無い** —— 参照実装は `blocks` とは別の `fluid: Uint8Array` を読み、その**バイト符号化を所有しているのは流体シミュレーション側**である。詳細と決着に必要なものは §3.6 |
...
### 3.6 流体メッシュが保留のままである理由 —— 「まだやっていない」ではなく「入力が無い」

AO（M-10）と十字板（M-11）は保留を解いた。**流体は解かなかった。**
理由は作業量ではなく、**決着していない所有権**が 1 つ残っていることである。ここに正確に書いておく。

**メッシュの側は疑いなくこのリポジトリのものである。** 責務表の行が自分で述べているとおり、
mx-gameplay のものは**伝播ルール**（どのセルにどれだけ流体が来るか）であって、
**水面の高さと側面の形をジオメトリにすること**ではない。そこに争いは無い。

**問題は入力である。** 参照実装の `resolveFluidState`（`greedy-meshing-fluid-state.ts:45-72`）は
...
とはいえ前例はある。`ChunkView`（`domain/chunk-view.ts`）は mc-kernel の `Chunk` が
publish されるまでの**最小の構造型**として宣言されており、publish されたら 1 行で差し替わる。
流体バイトにも同じ扱いが取れる。**取れるが、取るかどうかは決めていない**ので、
決めていないことを決めたふりをしない。

**決着に必要なもの（どちらか）:**

1. 流体バイトの符号化を所有するリポジトリが決まり、そこが公開する。mc-meshing はそれを読む。
2. `ChunkView` の前例にしたがい、`fluid?: Readonly<Uint8Array>` を**最小の構造型として**
   ここに宣言し、`porting.md` に差し替え予定として記録する。

**移植する中身自体は分かっており、見積もりも出せる:**

| 要素 | 出典 | 備考 |
| --- | --- | --- |
| 水源の水面高 `14/16` | `greedy-meshing-fluid-state.ts:37` | コメントに理由あり（岸辺がガラス壁に見えないため） |
| セルの高さ `max(1/(maxLevel+1), 1 - level/(maxLevel+1))` | 同 :39-43 | `maxLevelFor` は `fluid-position-utils` にある |
| 4 隅の高さ = 周囲 2x2 列の平均 | 同 :89-113 | 「流れ方向」はこの 4 隅の傾きとして現れる |
| 同種流体が上にあれば高さ 1 | 同 :74-87 | 水没セルは満水で描く |
| 面の露出規則（ガラス越しの水槽は見える） | 同 :129-134 | `isFluidFaceOccluder` |
| 側面の下端が隣接流体の高さに合う | `greedy-meshing-fluids.ts:94-97` | 段差の側面だけを出す |

**そして流体 quad も `Quad` にはなれない** —— 4 隅の Y が別々の小数なので、
矩形記述子では表せない。十字板（M-11）と同じ理由で、`MeshLayers` に
もう 1 つのリストが必要になる。**十字板でその形は既に通してあるので、そこは新しい問題ではない。**
