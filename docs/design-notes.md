...
チャンクの角では空気セルからの接線サンプルが対角チャンクに落ちるため、
`ChunkNeighbours` は optional な対角 4 方向も受け取る。ロード済みなら遮蔽に含め、
未ロードなら境界処理の他の場所と同じく `AIR`、すなわち「開いているものとして扱う」。
座標やストアは依然として持たず、どのチャンクを渡すかはキーを所有する呼び出し側の責務である。
...
流体の上面は矩形ではなく**双一次パッチ**であり、4 隅の傾きが水面の形を表す。
...
対角の隣接がロード済みなら、チャンクの 4 隅でも 4 サンプルの角平均を取る。
対角フィールドは optional なので、未ロード時は従来どおり 3 サンプルで開境界として描く。
