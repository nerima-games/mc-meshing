 ...
 *
 * ---------------------------------------------------------------------------
 * For mc-meshing the allowed set is EMPTY, which reads as "kernel and nothing
 * else". Meshing is a pure transformation from chunk data to geometry buffers;
 * it takes its transparency sets by injection (`MeshConfig`) precisely so that
 * it never needs to know which repository owns the block table.
 *
 * In particular mc-meshing must never gain a Three.js dependency. The reference
 * implementation kept `packages/world` free of Three.js imports and that
 * separation is being preserved here one level lower: geometry buffers are
 * plain typed arrays, and turning them into a `BufferGeometry` is mc-render's
 * job.
 *
...
export const REPOSITORY_POLICY = {
  thisPackage: '@nerima-games/mc-meshing',
